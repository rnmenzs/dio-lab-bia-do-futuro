...
**Fluxo:** ao abrir o app, os dois JSONs de `data/` são carregados no system prompt e o agente envia a primeira mensagem sozinho, com base nas metas do cliente (comportamento proativo). A cada resposta do LLM, o código valida duas coisas: todo produto recomendado deve ser um dos 5 do catálogo **e** ter risco compatível com o perfil do cliente; além disso, uma lista de termos proibidos (produtos comuns fora do catálogo, ex: poupança, COE, criptomoedas) bloqueia respostas que os citem. Resposta reprovada é descartada, e o agente responde com a mensagem de limitação.
...
| Validação | Produto citado deve ser do catálogo e compatível com o risco do cliente; termos proibidos (produtos externos) descartam a resposta |
